...
def process_data_vulcan_cnn(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    if zeroPadding:
...
def process_data_fetch_cnn(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    if zeroPadding:
        num_data = mat_data['Log']['plotTime'][0][0].shape[0]
    ...
    for cnt in range(num_data):
        ...
        for cnt2 in range(pred_window):
            ...
            if (1.001*for_cmd[ind-1+cnt2]-0.1832*lat_cmd[ind-1+cnt2]>1) or (1.001*for_cmd[ind-1+cnt2]+0.1832*lat_cmd[ind-1+cnt2]>1):
            #if (1.0*for_cmd[ind-1+cnt2]-0.1833*lat_cmd[ind-1+cnt2]>1) or (1.0*for_cmd[ind-1+cnt2]+0.1833*lat_cmd[ind-1+cnt2]>1):
                over_limit = True
                break
        if over_limit:
    ...
    print '\t# of data over the limit : %d' % over_limit_cnt

    # result return
    if over_limit_cnt > 0 and makeTrainValid:
        return [(train_data_in[0:train_cnt,:],train_data_out[0:train_cnt,:]),(valid_data_in[0:valid_cnt,:],valid_data_out[0:valid_cnt,:])]
    elif over_limit_cnt > 0:
        return (data_in[0:data_cnt,:], data_out[0:data_cnt,:])
    elif makeTrainValid:
        return [(train_data_in,train_data_out),(valid_data_in,valid_data_out)]
    else:
        return (data_in, data_out)
...
def process_data_fetch_cnn2(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    base_size = 0.508
    if zeroPadding:
        num_data = mat_data['Log']['plotTime'][0][0].shape[0]
        for_cmd, lat_cmd, lin_vel, ang_vel = np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window)
    ...
    for cnt in range(num_data):
        ...
        for cnt2 in range(hist_window):
            tmp[2*cnt2] = lin_vel[ind-hist_window+cnt2]-0.5*base_size*ang_vel[ind-hist_window+cnt2]
            tmp[2*cnt2+1] = lin_vel[ind-hist_window+cnt2]+0.5*base_size*ang_vel[ind-hist_window+cnt2]

        for cnt2 in range(pred_window):
            tmp[2*hist_window+2*cnt2] = for_cmd[ind-1+cnt2]
            tmp[2*hist_window+2*cnt2+1] = lat_cmd[ind-1+cnt2]
            tmp2[2*cnt2] = lin_vel[ind+cnt2]-0.5*base_size*ang_vel[ind+cnt2]
            tmp2[2*cnt2+1] = lin_vel[ind+cnt2]+0.5*base_size*ang_vel[ind+cnt2]
            if (1.001*for_cmd[ind-1+cnt2]-0.1832*lat_cmd[ind-1+cnt2]>1) or (1.001*for_cmd[ind-1+cnt2]+0.1832*lat_cmd[ind-1+cnt2]>1):
            #if (1.0*for_cmd[ind-1+cnt2]-0.1833*lat_cmd[ind-1+cnt2]>1) or (1.0*for_cmd[ind-1+cnt2]+0.1833*lat_cmd[ind-1+cnt2]>1):
                over_limit = True
                break
...
def process_data_magicbot_cnn(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    if zeroPadding:
        num_data = mat_data['Log']['plotTime'][0][0].shape[0]
...
#            / left wheel speed[ind-hist_window+1], right wheel speed[ind-hist_window+1]
#            ... / left wheel speed[ind-1], right wheel speed[ind-1]
#            / joystick forward[ind-hist_window], joystick left[ind-hist_window]
#            / joystick forward[ind-hist_window+1], joystick left[ind-hist_window+1]
#            ... / joystick forward[ind+pred_window-2], joystick left[ind+pred_window-2]
#
#   data_y : left wheel speed[ind], right wheel speed[ind] 
#            / left wheel speed[ind+1], right wheel speed[ind+1]
...
def process_data_vulcan_rnn(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    if zeroPadding:
        num_data = mat_data['Log']['plotTime'][0][0].shape[0]
        for_cmd, lat_cmd, left_wheel, right_wheel = np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window)
        for_cmd[hist_window:num_data+hist_window] = mat_data['Log']['forwardCommand'][0][0][:,0]
    ...
    if makeTrainValid:
        data_split_cnt = int(num_data*0.92)
        #train_data_in, train_data_out = np.zeros((data_split_cnt, 4*hist_window+2*pred_window-2)), np.zeros((data_split_cnt,2*pred_window))
        #valid_data_in, valid_data_out = np.zeros((num_data-data_split_cnt, 4*hist_window+2*pred_window-2)), np.zeros((num_data-data_split_cnt,2*pred_window))
        train_data_in, train_data_out = np.zeros((data_split_cnt, 2*hist_window+2*pred_window)), np.zeros((data_split_cnt,2*pred_window))
        valid_data_in, valid_data_out = np.zeros((num_data-data_split_cnt, 2*hist_window+2*pred_window)), np.zeros((num_data-data_split_cnt,2*pred_window))
        train_cnt, valid_cnt = 0, 0
    else:
        #data_in, data_out = np.zeros((num_data, 4*hist_window+2*pred_window-2)), np.zeros((num_data,2*pred_window))
        data_in, data_out = np.zeros((num_data, 2*hist_window+2*pred_window)), np.zeros((num_data,2*pred_window))
        data_cnt = 0

    # data re-allocation
    for cnt in range(num_data):
        #tmp, tmp2 = np.zeros(4*hist_window+2*pred_window-2), np.zeros(2*pred_window)
        tmp, tmp2 = np.zeros(2*hist_window+2*pred_window), np.zeros(2*pred_window)
        if mixData:
            ind = data_index[cnt] + hist_window
        else:
        ...
        for cnt2 in range(hist_window):
            tmp[2*cnt2] = left_wheel[ind-hist_window+cnt2]
            tmp[2*cnt2+1] = right_wheel[ind-hist_window+cnt2]
            #tmp[2*(hist_window+cnt2)] = for_cmd[ind-hist_window+cnt2]
            #tmp[2*(hist_window+cnt2)+1] = lat_cmd[ind-hist_window+cnt2]
        '''
        tmp[0:hist_window] = left_wheel[ind-hist_window:ind]
        tmp[hist_window:2*hist_window] = right_wheel[ind-hist_window:ind]
        '''
        ...
        for cnt2 in range(pred_window):
            tmp[2*hist_window+2*cnt2] = for_cmd[ind-1+cnt2]
            tmp[2*hist_window+2*cnt2+1] = lat_cmd[ind-1+cnt2]
            #tmp[4*hist_window-2+2*cnt2] = for_cmd[ind-1+cnt2]
            #tmp[4*hist_window-2+2*cnt2+1] = lat_cmd[ind-1+cnt2]
            tmp2[2*cnt2] = left_wheel[ind+cnt2]
            tmp2[2*cnt2+1] = right_wheel[ind+cnt2]
...
def process_data_magicbot_rnn(mat_data, hist_window, pred_window, useSmoothed, mixData=True, makeTrainValid=False, zeroPadding=False):
    if zeroPadding:
        num_data = mat_data['Log']['plotTime'][0][0].shape[0]
        left_cmd, right_cmd, left_wheel, right_wheel = np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window), np.zeros(num_data+hist_window)
        left_cmd[hist_window:num_data+hist_window] = mat_data['Log']['leftWheelCommand'][0][0][:,0]
    ...
    for cnt in range(num_data):
        ...
        for cnt2 in range(hist_window):
            tmp[2*cnt2] = left_wheel[ind-hist_window+cnt2]
            tmp[2*cnt2+1] = right_wheel[ind-hist_window+cnt2]
            #tmp[2*(hist_window+cnt2)] = left_cmd[ind-hist_window+cnt2]
            #tmp[2*(hist_window+cnt2)+1] = right_cmd[ind-hist_window+cnt2]
        '''
        tmp[0:hist_window] = left_wheel[ind-hist_window:ind]
        tmp[hist_window:2*hist_window] = right_wheel[ind-hist_window:ind]
        '''
        ...
        for cnt2 in range(pred_window):
            tmp[2*hist_window+2*cnt2] = left_cmd[ind-1+cnt2]
            tmp[2*hist_window+2*cnt2+1] = right_cmd[ind-1+cnt2]
            #tmp[4*hist_window-2+2*cnt2] = left_cmd[ind-1+cnt2]
            #tmp[4*hist_window-2+2*cnt2+1] = right_cmd[ind-1+cnt2]
            tmp2[2*cnt2] = left_wheel[ind+cnt2]
            tmp2[2*cnt2+1] = right_wheel[ind+cnt2]
...
#def load_data_vulcan_rnn(data_file_name, hist_window, pred_window, useSmoothed=True, datadir=None, testfilelist=None, excludefilelist=None):
def load_data(data_file_name, hist_window, pred_window, useSmoothed=True, datadir=None, testfilelist=None, excludefilelist=None, robot_type='Vulcan', model_type='CNN'):
    data_path = os.getcwd()
    datafilelist = os.listdir(data_path)

    if (robot_type == 'Vulcan') and (model_type == 'CNN'):
        process_data_func = process_data_vulcan_cnn
        print "data function for Vulcan and CNN is called"
    elif (robot_type == 'Vulcan') and (model_type == 'RNN'):
        process_data_func = process_data_vulcan_rnn
        print "data function for Vulcan and RNN is called"
    elif (robot_type == 'Fetch') and (model_type == 'CNN'):
        process_data_func = process_data_fetch_cnn
        print "1st type of data function for Fetch and CNN is called"
    elif (robot_type == 'Fetch2') and (model_type == 'CNN'):
        process_data_func = process_data_fetch_cnn2
        print "2nd type of data function for Fetch and CNN is called"
    elif (robot_type == 'MagicBot') and (model_type == 'CNN'):
        process_data_func = process_data_magicbot_cnn
        print "data function for MagicBot and CNN is called"
    elif (robot_type == 'MagicBot') and (model_type == 'RNN'):
        process_data_func = process_data_magicbot_rnn
        print "data function for MagicBot and RNN is called"
    else:
    ...
    if ((datadir is None) or (testfilelist is None)) or (data_file_name in datafilelist):
        ...
        with open(data_file_name, 'rb') as f:
            try:
                ...
                print "\ttrain %d, valid %d, test %d, final test %d" % (train_set[0].shape[0], valid_set[0].shape[0], test_set[0].shape[0], final_test_set[0].shape[0])
            except:
        ...
        num_train_in, num_train_out = train_data_in.shape[1], train_data_out.shape[1]
        tmp_train_shuffle = np.concatenate((train_data_in, train_data_out), axis=1)
        np.random.shuffle(tmp_train_shuffle)
        np.random.shuffle(tmp_train_shuffle)
        np.random.shuffle(tmp_train_shuffle)
        train_data_in, train_data_out = tmp_train_shuffle[:, 0:num_train_in], tmp_train_shuffle[:, num_train_in:(num_train_in+num_train_out)]
